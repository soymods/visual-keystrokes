...
public final class VisualKeystrokesEditorScreen extends Screen {
    ...
    public VisualKeystrokesEditorScreen(OverlayConfig config) {
    ...
    public boolean mouseClicked(Click click, boolean doubleClick) {
        ...
        if (handleSidebarSearchClick(click, doubleClick)) {
            return true;
        }

        if (handleHeaderClick(click.x(), click.y())) {
            return true;
        }

        if (infoOpen && handleInfoPopupClick(click.x(), click.y())) {
            return true;
        }

        if (handleSettingsButtonClick(click.x(), click.y())) {
            return true;
        }

        if (settingsOpen && handleSettingsPopupClick(click.x(), click.y())) {
            return true;
        }

        if (handleSidebarToggleClick(click.x(), click.y())) {
            return true;
        }

        if (handleResetClick(click.x(), click.y())) {
            return true;
        }
        ...
        double overlayX = toOverlayX(click.x());
        double overlayY = toOverlayY(click.y());
    ...
    public boolean mouseReleased(Click click) {
        if (dragMode != DragMode.NONE) {
            if (dragMode == DragMode.MOVE && !selectedGroups.isEmpty() && isInSidebarArea(click.x(), click.y())) {
    ...
    private boolean handleSidebarSearchClick(Click click, boolean doubleClick) {
        if (sidebarProgress <= 0.01f) {
            return false;
        }
        int sidebarWidth = (int) (SIDEBAR_WIDTH * sidebarProgress);
        ...
        int searchY = HEADER_Y + 14;
        int searchWidth = sidebarWidth - SIDEBAR_PADDING * 2;
        if (isPointInside(click.x(), click.y(), searchX, searchY, searchWidth, SEARCH_HEIGHT)) {
    ...
    private void drawTrashIcon(DrawContext context, Bounds bounds) {
        int trashX = bounds.x + bounds.width + 5;
