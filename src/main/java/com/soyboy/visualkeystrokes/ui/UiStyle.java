package com.soyboy.visualkeystrokes.ui;

import net.minecraft.client.font.TextRenderer;
import net.minecraft.client.gui.DrawContext;

public final class UiStyle {
    public static final int BG_BASE = 0x400B0D11;
    public static final int BG_SHADE = 0x2207090D;
    ...
    public static void drawToggle(DrawContext context, int x, int y, int width, int height, boolean enabled, boolean hovered) {
        drawToggle(context, x, y, width, height, enabled ? 1.0f : 0.0f, hovered);
    }

    public static void drawToggle(DrawContext context, int x, int y, int width, int height, float progress, boolean hovered) {
        ...
        int minX = x + 2;
        int maxX = x + width - knobSize - 2;
        int knobX = minX + Math.round((maxX - minX) * Math.max(0.0f, Math.min(1.0f, progress)));
        int knobY = y + 2;
        context.fill(knobX, knobY, knobX + knobSize, knobY + knobSize, lerpColor(ACCENT_RED, ACCENT_GREEN, progress));
